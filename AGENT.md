...
- **File structure:**
  ...
  - `run_all.py` — headless runner: `discover_methods` → simulate → price N → scorecards → export; writes run provenance via `export_config(cfg, scen)`. Flags: `--quick`, `--scenarios`, `--regimes`, `--seed`, `--ev-share` (MIX scenario EV mix, e.g. 0.30), `--tpo-share` (book TPO share, moved from Comprehensive), `--no-excel`, `--excel-scen`. Mix overrides are one-off — CFG defaults untouched. Books are written to `shared/` per scenario and never accumulated in RAM; export reloads from `shared/` (Excel first, CSV fallback to `shared/export_<SCEN>/` on MemoryError)
  - `run_stress.py` — stress runner: JSON scenario groups × seeds × regimes, prints mean-LR and P(LR>75%) pivots, saves tidy results per group + combined to `shared/stress_results/<group>_<stamp>.csv/.json`. Flags: `--grid` (directory or file, default `scenarios`), `--seeds`, `--scenarios`, `--regimes`, `--vehicle`, `--quick`, `--no-save`
  - `scenarios/*.json` — EDIT ME: stress scenario groups, one JSON list per file (`name`, `cfg` DGP overrides, `card` pricing overrides, optional `vehicle`) keeping the dated groups `01_claims_inflation` / `02_severity` / `03_portfolio_mix` / `04_pricing_response`. Levers: `claim_frequency_base`, `severity_multiplier`, `ev_severity_factor`, `coverage_pct`, `entrant_frac`; pricing: `expense_loading`, `tpo_loading`, `tpo_sa_pct`, `risk_step`, `train_book_seed`
  - `voltvision/` — helper package:
    - `config.py` — CFG, tariff tables, `COLS` canonical order, scenario mix, and self-export: `export_config(path=None, cfg=None, scen=None)` dumps every constant + timestamp to `shared/config_export.json` (callers pass their run's `cfg`/`scen` so one-off overrides are recorded; tuples serialise as strings, not reloadable)
    - `simulate.py` — premium-independent engine: `gen()` (risk attributes only — no claims, no premium), `claim_lambda()`, `loading()`, `simulate()` (the bread and butter)
    - `pricing.py` — CONNECTOR only: `RateCard`, registry, `quote`/`price_many`/`api_quote`, shared primitives (`encode_features`, `fit_frequency`, `severity_table`), reporting (`lr`, `lr_by`, `rho`, `retained_lr`, `summary`, `compare_all`, `reg_label`, `reg_color`), `discover_methods`
  ...
  - `shared/` — generated artifacts exchanged between notebooks (`sim_<SCEN>`, `priced_<SCEN>_<regime>`, `config_export.json`, `stress_results/`). Not committed; never hand-edit
...
- **Canonical column order (`voltvision.config.COLS`):** `POLID, COVERAGE_TYPE, SUM_ASSURED, REGION, VEHICLE_TYPE, ENGINE_CAPACITY, DRIVER_AGE_CAT, DRIVER_AGE, CAR_AGE, DRIVER_GENDER, FLOOD_RISK, THEFT_RISK, NCD_LEVEL_PRICED, NCD_LEVEL, NCD_YEARS, CLAIM_LAMBDA, SIM_YEAR, CLAIM_COUNT, CLAIM_OCCURRED, CLAIM_AMOUNT, CLAIM_PERIL, RENEWAL_PROB, RENEWED, COHORT_YEAR, BEHAVIOR_RISK, telematics_score` — priced books append `FINAL_PREMIUM_SST`. No `BASIC_PREMIUM` in the book (tariff base computed by the tariff pricer `02a` from attributes); no `TOTAL_LOADING` either (tariff pricer recomputes it via `loading()`)
- **Invariants:** simulation is premium-independent (price only post-simulation); all regimes share identical claims (same book, same seed); `NCD_LEVEL_PRICED` is the pre-claim snapshot, `NCD_LEVEL` is post-update; never show one regime alone
...
- Stress run: `python run_stress.py` (edit `scenarios/*.json` first; `--quick --seeds 0` smoke, `--scenarios base,freq_x1.22`, `--vehicle EV`). ML legs retrain on the scenario's stressed DGP (insurer-aware stress); tariff uses no claims data
...
- Keep `ENGINE_CAPACITY` in history output (full notebook drops it); never silently re-add `BASIC_PREMIUM` to the book — tariff money belongs to the tariff pricer (02a `basic_premium()` from `BANDS`/`BASIC_COMP`/`BASIC_TPO`/`PER_EXTRA`)
...
## Boundaries
- ✅ **Always do:** Work in `01`–`04`, `run_all.py`, `run_stress.py`, `scenarios/*.json`, `voltvision/`; keep calculations in the 02x notebooks (connector owns no math); preserve canonical column order; keep all regimes side by side in every table/figure; add comments per the readability standard
