...
- Telematics is EV-only: device data only exists for EVs, so `telematics_score` is `NaN` for ICE rows (the latent `BEHAVIOR_RISK` stays for every vehicle and still drives frequency + retention behaviour penalty). The `telem` method holds TWO models: EV (`GLM features + telematics_score`, trained on a full-EV book) and ICE (`GLM features`, trained on a full-ICE book); severity merges the two books.
