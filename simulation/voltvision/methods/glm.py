"""GLM pricing method — Poisson frequency x observed severity / target LR.
...
# NCD_LEVEL is deliberately absent: the discount is statutory and applied to
# the model output (like the tariff), never learned as a rating factor.
...
def train_model(book, card, cfg, base_cfg=None):
    """Fit the frequency model exactly as pricing does.

    Returns (model, sev, covsev, training_rows) — the analysis notebook uses
    this so SHAP explanations are attributed to the SAME fitted model the
    pricer used (single code path, no duplication).
    """
...
register_pricer('glm', price_glm, card=CARD, info={
    'label': 'GLM',
    'color': '#f59e0b',
    'formula': ('Poisson freq (features above) x avg severity / target_lr '
                'x (1-NCD, TPO exempt) x risk_step^flags'),
})
